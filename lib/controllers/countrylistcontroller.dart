...
class CountryListController {
  ...
  Future<List<CountryModel>> fetchData() async {
    try {
      ...
      return data.map((item) => CountryModel.fromJson(item)).toList(); //convert data list of dynamic to the country model object using factory method by mapping them to list
    } catch (e) {
      print("Error: $e");
      return [];
    }
